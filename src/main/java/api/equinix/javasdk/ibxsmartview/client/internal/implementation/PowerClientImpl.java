...
public class PowerClientImpl extends ResourceClientBase<PowerReading, PowerReadingJson> implements PowerClient<PowerReading> {

    public PowerClientImpl(IBXSmartViewConfigImpl configClient) {
        super(configClient, "IBXSmartView", "Power", PowerReadingJson.class);
    }

    @Override
    protected PowerReading wrap(PowerReadingJson json) {
        return json;
    }

    public Page<PowerReading, PowerReadingJson> list(String ibx) {
        return listPagePath("ListPowerReadings", Map.of("ibx", ibx));
    }

    public PowerReadingJson getPowerReading(String ibx, String cabinetId) {
        return getOne("GetPowerReading", Map.of("ibx", ibx, "cabinetId", cabinetId));
    }
}
