...
public class SupportCaseClientImpl extends ResourceClientBase<SupportCase, SupportCaseJson> implements SupportCaseClient<SupportCase> {

    public SupportCaseClientImpl(CustomerPortalConfigImpl configClient) {
        super(configClient, "CustomerPortal", "SupportCases", SupportCaseJson.class);
    }

    @Override
    protected SupportCase wrap(SupportCaseJson json) {
        return new SupportCaseWrapper(json, this);
    }

    public Page<SupportCase, SupportCaseJson> list() {
        return listPage("ListSupportCases");
    }

    public SupportCaseJson getByUuid(String uuid) {
        return getOne("GetSupportCase", uuid);
    }

    public SupportCaseJson create(SupportCaseCreatorJson supportCaseCreatorJson) {
        return postOne("CreateSupportCase", supportCaseCreatorJson);
    }

    public SupportCaseJson update(String uuid, SupportCaseCreatorJson supportCaseCreatorJson) {
        return updateOne("UpdateSupportCase", uuid, supportCaseCreatorJson);
    }

    public SupportCaseJson delete(String uuid) {
        return deleteOne("UpdateSupportCase", uuid);
    }
